...
from utils.image_export import show_export_format
...
def graph_controls(chart_type, df, dropdown_options, template):
    ...
    if chart_type == 'Scatter plots':
        ...
        try:
            x_values = st.sidebar.selectbox(
                'X axis', index=length_of_options, options=dropdown_options)
            y_values = st.sidebar.selectbox(
                'Y axis', index=length_of_options, options=dropdown_options)
            color_value = st.sidebar.selectbox(
                "Color", index=length_of_options, options=dropdown_options)
            symbol_value = st.sidebar.selectbox(
                "Symbol", index=length_of_options, options=dropdown_options)
            size_value = st.sidebar.selectbox(
                "Size", index=length_of_options, options=dropdown_options)
            hover_name_value = st.sidebar.selectbox(
                "Hover name", index=length_of_options, options=dropdown_options)
            facet_row_value = st.sidebar.selectbox(
                "Facet row", index=length_of_options, options=dropdown_options,)
            ...
            log_x = st.sidebar.selectbox(
                'Log axis on x', options=[True, False])
            log_y = st.sidebar.selectbox(
                'Log axis on y', options=[True, False])
    ...
    if chart_type == 'Histogram':
        ...
        try:
            ...
            nbins = st.sidebar.number_input(
                label='Number of bins', min_value=2, value=5)
            color_value = st.sidebar.selectbox(
                "Color", index=length_of_options, options=dropdown_options)

            barmode = st.sidebar.selectbox(
                'bar mode', options=['group', 'overlay', 'relative'], index=2)
            marginal = st.sidebar.selectbox("Marginal", index=2, options=['rug', 'box', None,
                                                                          'violin', 'histogram'])
            barnorm = st.sidebar.selectbox(
                'Bar norm', options=[None, 'fraction', 'percent'], index=0)
            ...
            facet_column_value = st.sidebar.selectbox("Facet column", index=length_of_options,
                                                      options=dropdown_options)
            cummulative = st.sidebar.selectbox(
                'Cummulative', options=[False, True])
            log_x = st.sidebar.selectbox(
                'Log axis on x', options=[True, False])
            log_y = st.sidebar.selectbox(
                'Log axis on y', options=[True, False])
    ...
    if chart_type == 'Line plots':
        ...
        try:
            ...
            color_value = st.sidebar.selectbox(
                "Color", index=length_of_options, options=dropdown_options)
            line_group = st.sidebar.selectbox(
                "Line group", options=dropdown_options)
            line_dash = st.sidebar.selectbox(
                "Line dash", index=length_of_options, options=dropdown_options)
            hover_name_value = st.sidebar.selectbox(
                "Hover name", index=length_of_options, options=dropdown_options)
            facet_row_value = st.sidebar.selectbox(
                "Facet row", index=length_of_options, options=dropdown_options, )
    ...
    if chart_type == 'Box plots':
        ...
        try:
            ...
            color_value = st.sidebar.selectbox(
                "Color", index=length_of_options, options=dropdown_options)
            boxmode = st.sidebar.selectbox(
                'Violin mode', options=['group', 'overlay'])
            outliers = st.sidebar.selectbox('Show outliers', options=[
    ...
    if chart_type == 'Sunburst':
        ...
        try:
            path_value = st.sidebar.multiselect(
                label='Path', options=dropdown_options)
            color_value = st.sidebar.selectbox(
                label='Color', options=dropdown_options)
            value = st.sidebar.selectbox(
                "Value", index=length_of_options, options=dropdown_options)
            title = st.sidebar.text_input(label='Title of chart')
    ...
    if chart_type == 'Pie Charts':
        ...
        try:
            name_value = st.sidebar.selectbox(
                label='Name (Selected Column should be categorical)', options=dropdown_options)
            color_value = st.sidebar.selectbox(
                label='Color(Selected Column should be categorical)', options=dropdown_options)
            value = st.sidebar.selectbox(
                "Value", index=length_of_options, options=dropdown_options)
